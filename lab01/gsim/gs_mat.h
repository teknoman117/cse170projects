...
/*! \class GsMat gs_mat.h
    ...
    column-major or line-major format; however, the default
    behavior of the methods is to assume GsMat in column-major
    format for full compatibility with OpenGL. If line-major
...
class GsMat
 { public :
   ...
   public :
    ...
    void translation ( float tx, float ty, float tz, char fmt='C' );

    /*! Make GsMat be a translation transformation of v;
        fmt can be 'C' or 'L' for column (default) or line major format. */
    void translation ( const GsVec& v, char fmt='C' ) { translation(v.x,v.y,v.z,fmt); }

    /*! Set the translation fields of the matrix to be tx, ty, tz;
        fmt can be 'C' or 'L' for column (default) or line major format. */
    void setrans ( float tx, float ty, float tz, char fmt='C' );

    /*! Set the translation fields of the matrix to be v.x, v.y, v.z;
        fmt can be 'C' or 'L' for column (default) or line major format. */
    void setrans ( const GsVec& v, char fmt='C' ) { setrans(v.x,v.y,v.z,fmt); }

    /*! Get the translation fields of the matrix and put in tx, ty, tz;
        fmt can be 'C' or 'L' for column (default) or line major format. */
    void getrans ( float& tx, float& ty, float& tz, char fmt='C' ) const;

    /*! Get the translation fields of the matrix and put in v.x, v.y, v.z;
        fmt can be 'C' or 'L' for column (default) or line major format. */
    void getrans ( GsVec& v, char fmt='C' ) const { getrans(v.x,v.y,v.z,fmt); }

    /*! Pre-multiplies GsMat with a translation matrix of translation vector v;
        fmt can be 'C' or 'L' for column (default) or line major format. */
    void lcombtrans ( const GsVec& v, char fmt='C' );
    ...
    void rcombtrans ( const GsVec& v, char fmt='C' );
    ...
    void rot ( const GsVec& vec, float sa, float ca, char fmt='C' );

    /*! Rotation around an axis given an angle in radians.
        The given axis of rotation vec is not required to be normalized.
        Mode is 'C' for column-major (default) and 'L' for line-major format. */
    void rot ( const GsVec& vec, float radians, char fmt='C' );

    /*! Gives the rotation matrix that rotates one vector to another.
        Vectors are not required to be normalized.
        Mode is 'C' for column-major (default) and 'L' for line-major format. */
    void rot ( const GsVec& from, const GsVec& to, char fmt='C' );
    ...
    /*! Set GsMat to be the rigid transformation matrix that maps the three
        ...
        Mode is 'C' for column-major (default) and 'L' for line-major format. */
    void projxy ( GsVec v1, GsVec v2, GsVec v3, char fmt='C' );
